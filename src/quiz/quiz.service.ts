...
export class QuizService {
  ...
  private async _generateQuestionsFromChunks(
    ...
    const prompt = `
      ...
      [출력 규칙]
      - **중요**: 반드시 아래의 JSON 배열 포맷으로만 응답하세요.
      - **중요**: 시작과 끝에 \`\`\`json 이나 \`\`\` 같은 마크다운 문법을 쓰지 마십시오. 순수한 JSON 문자열만 출력하세요.
      - 다른 부가적인 설명(안녕하세요, 여기 있습니다 등)은 절대 포함하지 마십시오.
      - 정답(answer)과 상세한 해설(explanation)을 반드시 포함해야 합니다.
